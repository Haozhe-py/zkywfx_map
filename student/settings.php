            ...
            <div style="margin-top: 5%; height: 80%; width: 100%;">
                ...
                <div id="account-page" style="height: 100%; width: 100%; display: none;">
                    <div id="account-home">
                        ...
                        <br />
                        <button onclick="logOut();" style="color: white; background: red;">退出登录</button>
                    </div>
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            if ($action !== '') {
                ...
                if ($action === 'change_username') {
                    ...
                    if ($new_pwd !== $conf_pwd) {
                        echo "<script>window.alert('两次输入的密码不一致！');</script>";
                        exit();
                    }
                    try {
                        $manager->atomicUpdate(function ($data) use (&$cur_id, &$cur_pwd, &$new_pwd){
                            foreach ($data['users'] as &$user) {
                                if ($user['id']===$cur_id) {
